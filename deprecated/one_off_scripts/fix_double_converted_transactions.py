...
from deprecated.one_off_scripts._double_conversion_fix import (
    ...
    try:
        ...
        with YnabClient(api_key) as gateway:
            ...
            for account_alias in account_aliases:
                plan = build_double_conversion_fix_plan(
                    plan=config.plan,
                    gateway=gateway,
                ...
                for update in plan.updates:
                    old_amount = format_milliunits(update.old_amount_milliunits, places=3)
                    new_amount = format_milliunits(update.new_amount_milliunits, places=3)
                    print(
                        f"- {update.date.isoformat()} {update.account_alias}: "
                        f"{old_amount} -> {new_amount}"
                    )
                    print(f"  memo: {update.old_memo}")
                    print(f"  -> {update.new_memo}")
                if args.apply:
                    applied_writes += apply_double_conversion_fix_plan(
                        gateway=gateway,
                        plan=plan,
                    )
        ...
        print(f"Prepared fixes: {prepared_updates}")
        if args.apply:
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="fix_double_converted_transactions.py",
        description=(
            "Fix transactions that were converted twice and now contain both a legacy FX "
            "marker and a current [FX] marker."
        ),
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Limit the fix to one configured account alias. "
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Write the repaired amount and memo back to YNAB.",
    )
    return parser
